...
class AppSettingController extends Controller
{
    ...
    public function product_page_setting(Request $request)
    {
        ...
        if (isset($array['section']['image']['text']) && !is_array($array['section']['image']['text']) && gettype($array['section']['image']['text']) == 'object') {
            ...
            if (!$upload['error']) {
                if (!empty($array['section']['image']['image']) && $array['section']['image']['image'] != 'themes/grocery/assets/images/banner-image.png' && \File::exists(base_path($array['section']['image']['image']))) {
                    Utility::changeStorageLimit(\Auth::user()->creatorId(), $array['section']['image']['image']);
                }
                $array['section']['image']['text'] = $upload['data']['image_path'];
                $array['section']['image']['image'] = $upload['data']['image_path'];
            } else {
                return redirect()->back()->with('error', $upload['data']);
            }
        }
