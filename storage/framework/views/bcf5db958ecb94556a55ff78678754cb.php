<section class="home-banner-section" style="position: relative;<?php if(isset($option) && $option->is_hide == 1): ?> opacity: 0.5; <?php else: ?> opacity: 1; <?php endif; ?>" data-index="<?php echo e($option->order ?? ''); ?>" data-id="<?php echo e($option->order ?? ''); ?>" data-value="<?php echo e($option->id ?? ''); ?>" data-hide="<?php echo e($option->is_hide ?? ''); ?>" data-section="<?php echo e($option->section_name ?? ''); ?>"  data-store="<?php echo e($option->store_id ?? ''); ?>" data-theme="<?php echo e($option->theme_id ?? ''); ?>">
    
    <div class="mian-banner-image">
        <img src="<?php echo e(get_file($section->slider->section->background_image->image ?? '', $currentTheme)); ?>" class="img-fluid <?php echo e(($section->slider->section->background_image->slug ?? '').'_preview'); ?>" alt="main-banner">
    </div>
        ...
        <div class="banner-main-content d-flex align-items-center">
            ...
            <div class="banner-right-side">
                <div class="banner-slider">
                    <?php for($i = 0; $i < $section->slider->loop_number ?? 1; $i++): ?>
                        <div class="home-banner-content">
                            <div class="home-banner-content-inner">
                                <div class="new-labl" id="<?php echo e(($section->slider->section->button_second->slug ?? '').'_'. $i); ?>_preview">
                                    <?php echo $section->slider->section->button_second->text->{$i} ?? ""; ?>

                                </div>
                                <h2 class="h1" id="<?php echo e(($section->slider->section->title->slug ?? '').'_'. $i); ?>_preview">
                                    <?php echo $section->slider->section->title->text->{$i} ?? ""; ?>

                                </h2>
                                <div class="desk-wrapper">
                                    <p id="<?php echo e(($section->slider->section->description->slug ?? '').'_'. $i); ?>_preview"><?php echo ($section->slider->section->description->text->{$i} ?? ''); ?></p>
                                </div>
                                <a href="<?php echo e(route('page.product-list',  ['storeSlug' => $slug])); ?>" class="btn btn-white"
                                    tabindex="0" id="<?php echo e(($section->slider->section->button_first->slug ?? '').'_'. $i); ?>_preview">
                                    <span class="first-icon"><svg width="11" height="11" viewBox="0 0 11 11"
                                        ...
                                        <?php echo e(($section->slider->section->button_second->text->{$i} ?? '')); ?> <svg xmlns="http://www.w3.org/2000/svg" width="4"
...
<?php /**PATH B:\xampp\htdocs\ecommers\themes\grocery\views/front_end/sections/homepage/slider_section.blade.php ENDPATH**/ ?>